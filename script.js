...
function register() {
  ...
  if (!name || !email || !password) {
    error.innerText = "⚠️ عمّري الخانات الكل.";
    return;
  }
  ...
  alert("🎉 مرحبًا " + name + "! تم إنشاء حسابك.");
...
function renderBadges(user) {
  ...
  badges.forEach(function (badge) {
    ...
    if (!user.badges.includes(badge[1])) {
...
function updateDashboard() {
  ...
  if (welcome)
    welcome.innerText = "👋 مرحبًا " + user.name + "!";

  if (xp)
    xp.innerText = user.xp;

  if (level)
    level.innerText = calculateLevel(user.xp);

  if (challengesDone)
    challengesDone.innerText = user.completed.length;

  if (badges)
    badges.innerText = user.badges.length;

  if (progressText)
    progressText.innerText = user.xp + " XP";
...
function startChallenge(index) {
  ...
  document.getElementById("quizXP").innerText =
    "Challenge • +" + challenge.xp + " XP";

  document.getElementById("quizQuestion").innerText =
    challenge.question;

  document.getElementById("quizNumber").innerText =
    "السؤال 1 من 1";
...
function finishQuiz() {
  ...
  if (!user.completed.includes(currentChallenge)) {
    user.completed.push(currentChallenge);
...
function renderSafetyQuestion() {
  const question =
    safetyQuestions[currentSafetyQuestion];
...
function initializeAMAN() {
  if (
    localStorage.getItem("amanDark") === "true"
  ) {
